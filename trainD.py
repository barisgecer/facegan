...
class trainD(object):
    def __init__(self, config, real_image, syn_image, syn_label, image_3dmm, annot_3dmm):
        ...
        if (not config.train_generator) | (config.load_path != '') :
            ...
            self.pre_train_saver_avg = tf.train.Saver({k: v for k,v in variables_to_restore.items() if ('InceptionResnetV1' not in k) and ('centroids' not in k)})
            all_variables = tf.global_variables()
            #all_variables = {v for v in all_variables if v.name != self.centroids.name}
            self.pre_train_saver_all = tf.train.Saver({v for v in all_variables if ('InceptionResnetV1' not in v.name) and ('centroids' not in v.name)})
    ...
    def generate_dataset(self):
        ...
        for i in range(0,len(paths),self.config.batch_size):
            counter +=1
            if i + self.config.batch_size < len(paths):
                pa = paths[i:(i + self.config.batch_size)]
                labels_feed = labels[i:(i + self.config.batch_size)]
                begin_ind = 0
            else:
    ...
    def build_model(self):
        ...
        with tf.device('/cpu:0'):
            ...
            for i in range(self.config.num_gpu):
                ...
                with tf.device('/gpu:%d' % i):
                    ...
                    # Build Graph
                    # Generation
                    self.x = self.syn_image[gpu_ind]
                    x = norm_img(self.x)
                    self.x_all.append(x)
                    self.config.method_c = 'softmax'
                    ...
                    def D(scope_name, x, real_image_norm):
                        with tf.variable_scope(scope_name):
                            def _build_discrim(layer, name, reuse=False):
                                with tf.variable_scope("discriminator", reuse=reuse) as sc: # 108
                                    layer = conv2d(layer, 16, 3, 2, scope="conv_1", name=name) # 54
                                    layer = conv2d(layer, 16, 3, 2, scope="conv_2", name=name) # 27
                                    layer = max_pool2d(layer, 3, 2, scope="max_1", name=name) # 14
                                    layer = conv2d(layer, 32, 3, 2, scope="conv_4", name=name) # 7
                                    layer = max_pool2d(layer, 3, 2, scope="max_2", name=name) # 4
                                    layer = conv2d(layer, 32, 3, 2, scope="conv_6", name=name) # 2
                                    layer = max_pool2d(layer, 2, 2, scope="max_3", name=name) # 1
                                    logits = conv2d(layer, 2, 1, 1, scope="conv_8", name=name)
                            ...
                            D_y, D_y_logits = \
                                _build_discrim(real_image_norm, name="D_y")
                            D_R_x, D_R_x_logits = \
                                _build_discrim(x, name="D_R_x", reuse=True)

                            refiner_d_loss = log_loss(
                                D_R_x_logits, fake_label(D_R_x_logits), "refiner_d_loss")
                            synthetic_d_loss = log_loss(
                                D_y_logits, real_label(D_y_logits), "synthetic_d_loss")

                        variables = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=scope_name)

                        return tf.reduce_mean(refiner_d_loss + synthetic_d_loss, name="discrim_loss"), refiner_d_loss + synthetic_d_loss, variables, tf.reduce_mean(refiner_d_loss), tf.reduce_mean(synthetic_d_loss)

                    self.d_loss, d_loss_each, d_vars, self.d_loss_fake,self.d_loss_real = D('Disc',x,real_image_norm)

                    g_optim = g_optimizer.compute_gradients(self.d_loss + self.c_loss, var_list=d_vars+self.C_logits_var+self.C_var)
                    ...
                    if i == self.config.num_gpu-1:
                        self.summary_op = tf.summary.merge([
                            ...
                            tf.summary.scalar("loss/d_loss", self.d_loss_fake),
                            tf.summary.scalar("loss/d_loss", self.d_loss_real),
            ...
            with tf.control_dependencies([train_op_G, variables_averages_op]):
        ...
        return d_vars + [self.centroids] , self.C_var, variable_averages#, self.G_inv_var, self.G_var
